...
You can now convert the Docker image archive to an Apptainer `.sif` image on Kestrel with the following `build` command:

```
cd ${LOCAL_SCRATCH}
module load apptainer/1.1.9
apptainer build simple_python3.sif simple_python3.tar.gz
```
