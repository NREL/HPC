...
int main(int argc, char *argv[]) {
	...
	int mysize=2048;
	int order=1;
	if (argc > 1) mysize=std::stoi(argv[1]);
	if (argc > 2) order=std::stoi(argv[2]);

    //dim_t fft = {256, 256, 256};
    //dim_t fft = {1024,2048,2048};
    dim_t fft = {(unsigned long)mysize,(unsigned long)mysize,(unsigned long)mysize};
    // can be {0, 0} to run on single-GPU system or if GPUs are not of same architecture
    //gpus_t gpus = {0, 1};
    gpus_t gpus = {0, 1,2,3};
    std::cout << "size " << fft[0] << "," << fft[1] <<"," << fft[2] << std::endl;
    ...
    std::cout << "// skip verify results" << std::endl;
/*
    double error{};
    double ref{};
